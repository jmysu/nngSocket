#!/usr/bin/python
"""
    NNG BUS simulation with pure Python sockets
   +---------------------------------------------------------------+ 
    0                   1                   2                   3
...
nngHeaderBus = bytearray(b'\x00\x53\x50\x00\x00\x70\x00\x00')
...
while True:  
    ...
    try:
        ...
        while True:
            ...
            if (data.startswith(nngHeaderPub)) : 
                print("Found NNG Pub Header!")
                connection.send(nngHeaderSub)
            if (data.startswith(nngHeaderBus)) :  
                print("Found NNG Bus Header!")
                connection.send(nngHeaderBus)
